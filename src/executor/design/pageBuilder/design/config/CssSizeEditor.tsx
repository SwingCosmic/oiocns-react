...
export const CSSUnits = ["px", "pt", "cm", "in", "em", "rem", "vh", "vw", "vmin", "vmax", "%"] as const;
...
const CssSizeEditor : IExistTypeEditor<string, { }> = ({ value, onChange }) => {
  ...
  const prefix = (
    <Select value={type.value} onChange={v => {
  ...
  const postfix = (
    <Select value={unit.value} onChange={v => unit.value = v} 
      disabled={type.value != "value"}>
      {CSSUnits.map(v => <Select.Option key={v} value={v}>{v}</Select.Option>)}
    </Select>
  );
  return (
    <InputNumber 
