 ...
 */
export type ExtractToType<T extends TypeMeta> = T['type'] extends 'string'
  ? string
  ...
  : T['type'] extends 'date'
  ? string
  : T extends EnumTypeMeta<infer R>
  ? R
  : T extends ArrayTypeMeta
  ? ExtractToType<T['elementType']>[]
  : T extends ObjectTypeMeta
  ? {
      [P in keyof T['properties']]: ExtractToType<T['properties'][P]>;
    }
  : T extends ExistTypeMeta<infer R>
  ? R
  : any;
